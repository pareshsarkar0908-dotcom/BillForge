...
async function handleForgotPassword(event) {
  ...
  if (supabaseClient) {
    ...
    setAuthView("reset");
    setAuthMessage("Reset code sent. Enter the 6-digit code from your email.");
  } else {
...
async function handleResetPassword(event) {
  ...
  if (!/^[0-9]{6}$/.test(resetCode) && !isResetRequest()) {
    setAuthMessage("Enter the 6-digit reset code from your email.");
    return;
  }
...
async function initPasswordResetView() {
  ...
  setAuthMessage(hasRecoveryToken ? "Enter your new password to finish resetting your account." : "Enter the 6-digit reset code from your email.");
  return true;
}
