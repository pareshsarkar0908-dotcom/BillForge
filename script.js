...
const resetCodePattern = /^[0-9]{6,8}$/;
...
function resetRedirectUrl() {
  if (window.location.hostname === "billforge.net" || window.location.hostname === "www.billforge.net") {
    return "https://billforge.net/?reset=1";
  }
...
async function handleForgotPassword(event) {
  ...
  if (supabaseClient) {
    ...
    sessionStorage.setItem("billforge_reset_email", email);
    document.querySelector("#resetEmail").value = email;
    setAuthView("reset");
    setAuthMessage("Reset code sent. Enter the code from your email.");
  } else {
...
async function handleResetPassword(event) {
  ...
  const resetCode = document.querySelector("#resetCode")?.value.trim() || "";
  ...
  if (!resetCodePattern.test(resetCode) && !isResetRequest()) {
    setAuthMessage("Enter the reset code from your email.");
    return;
  }
  ...
  try {
    ...
    if (resetCodePattern.test(resetCode)) {
      ...
      const { error } = await withTimeout(
        ...
        "Could not verify the reset code. Please request a new code.",
        15000
      );
      if (error) throw error;
      activated = true;
    } else {
      activated = await activateRecoverySession();
    }
    if (!activated) {
      setAuthMessage("This reset code or link is expired. Please request a new password reset email.");
      setAuthView("forgot");
      return;
    }
...
async function initPasswordResetView() {
  ...
  showPage("login", false);
  setAuthView("reset");
  const storedEmail = sessionStorage.getItem("billforge_reset_email") || "";
  if (storedEmail) document.querySelector("#resetEmail").value = storedEmail;
  setAuthMessage(hasRecoveryToken ? "Enter your new password to finish resetting your account." : "Enter the reset code from your email.");
  return true;
}
