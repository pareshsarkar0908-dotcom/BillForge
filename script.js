...
function resetRedirectUrl() {
  if (window.location.hostname === "billforge.net" || window.location.hostname === "www.billforge.net") {
    return "https://www.billforge.net/?reset=1";
  }
...
async function handleResetPassword(event) {
  ...
  try {
    const activated = await activateRecoverySession();
    if (!activated) {
      setAuthMessage("This reset link is expired or not active. Please request a new password reset email.");
      setAuthView("forgot");
      return;
    }
...
async function initPasswordResetView() {
  const params = resetLinkParams();
  const hasRecoveryToken = !!(params.code || params.type === "recovery" || params.accessToken);
  const isRecovery = params.reset === "1" || hasRecoveryToken;
  if (!isRecovery) return;
  showPage("login", false);
  if (params.reset === "1" && !hasRecoveryToken) {
    setAuthView("forgot");
    setAuthMessage("This reset link did not include a valid reset code. Please request a new password reset email.");
    return;
  }
  setAuthView("reset");
  setAuthMessage("Enter your new password to finish resetting your account.");
  activateRecoverySession().catch(error => {
    setAuthMessage(error.message || "Could not activate the reset link. Please request a new password reset email.");
  });
}
