...
async function handleResetPassword(event) {
  ...
  try {
    ...
    let sessionResult = await withTimeout(supabaseClient.auth.getSession(), "Could not check the reset session.");
    if (!sessionResult.data?.session && code) {
      await withTimeout(supabaseClient.auth.exchangeCodeForSession(code), "Could not activate the reset link.");
      sessionResult = await withTimeout(supabaseClient.auth.getSession(), "Could not check the reset session.");
    }
    if (!sessionResult.data?.session && !passwordRecoveryReady) {
...
async function initPasswordResetView() {
  ...
  const isRecovery = params.get("reset") === "1" || params.has("code") || hash.includes("type=recovery") || hash.includes("access_token=");
  if (!isRecovery) return;
  if (supabaseClient && params.has("code")) {
    await withTimeout(
      supabaseClient.auth.exchangeCodeForSession(params.get("code")),
