    ...
    try {
      ...
      if (data?.session?.user) {
        ...
        const loginResult = await withTimeout(
          ...
          "Account created, but login took too long. Please login manually."
        );
...
async function handleResetPassword(event) {
  ...
  try {
    const sessionResult = await withTimeout(
      ...
      "Could not check the reset session. Please open the newest reset email link again.",
      10000
    );
    if (!sessionResult?.data?.session && isResetRequest()) {
