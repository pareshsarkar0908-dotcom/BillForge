...
function updateAccountUi() {
  ...
  document.querySelector("#accountStatus").textContent = state.user ? `${plan?.name || "No plan"}: ${used}/${limit} PDFs` : "Guest";
...
async function handleForgotPassword(event) {
  ...
  if (supabaseClient) {
    ...
    setAuthMessage(error ? error.message : "Password reset link sent.");
  } else {
...
async function startPayment(planId) {
  ...
  if (!window.Razorpay || appConfig.razorpayKeyId.includes("YOUR_")) {
    setAuthMessage("Add your Razorpay Key ID in script.js. Production payments also need a backend order endpoint.");
    return;
  }
