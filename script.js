...
async function handleResetPassword(event) {
  ...
  if (!sessionResult.data?.session && code) {
    await supabaseClient.auth.exchangeCodeForSession(code).catch(() => null);
...
async function initPasswordResetView() {
  ...
  if (params.has("code") && supabaseClient) {
    await supabaseClient.auth.exchangeCodeForSession(params.get("code")).catch(() => null);
  }
